# Resilience and error handling — deep dives
...
**Choosing values.** Timeout must be below the caller's, minus processing overhead. Typical layering: API gateway 30 s, service-to-service 10 s, DB query 5 s. Less than 500 ms risks false timeouts; more than 30 s ties up requests too long. Set per call-site; reads can tolerate shorter than writes; fan-out endpoints need longer than point-lookups.
