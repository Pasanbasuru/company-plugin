...
| `docs/superpowers/skill-authoring-guide.md` | Receives merged `skill-authoring/SKILL.md` content; new section pointing at `templates/new-skill-template.md` (scaffold) and `templates/baseline-standards.md` (standards reference). **Merge strategy: interleave by topic** (Option B per spec review). The two files overlap ~70% but differ in form (the guide is procedural docs; the skill is enforcement-shaped with numbered Core rules + `*Why:*` lines). Produce one coherent document by: (1) keeping the guide's overall spine — file layout, section order, section-by-section requirements; (2) for each topic that appears in both (description requirements, four-section Review checklist shape, size targets, Interactions, red flags), pick the strongest wording from either source; (3) graft the skill's `*Why:*` lines onto the corresponding guide rules where the skill has them and the guide doesn't; (4) graft the skill's "TRIGGER when / SKIP when" guidance into the description-writing section; (5) lift the skill's "Authoring flow" (brainstorming → writing-skills → company conventions → skill-verification) as a new top-level section near the top of the guide; (6) keep the guide's "Self-review before commit" section verbatim (the skill doesn't have this); (7) keep the guide's "What every skill assumes" but update its `_baseline` reference to `templates/baseline-standards.md`; (8) add the new templates-pointer section near the top, just under "Authoring flow," that names `templates/new-skill-template.md` as the scaffold to copy and `templates/baseline-standards.md` as the standards reference. End state: single document, no duplicated rule statements. | Two parallel structures in one file (Option A) would have read as redundant; wholesale replace (Option C) would have lost the guide's procedural framing and the "Self-review before commit" + "What every skill assumes" sections. Interleaving costs ~1 hour of editorial work in commit 5 vs. ~15 minutes for append-and-dedup, but produces a clean reference document for future skill authors. |
...
| Operation | Path | Change |
|---|---|---|
| Modify | `plugin/skills/change-risk-evaluation/SKILL.md` | Absorb unique rules + review-checklist content from `regression-risk-check` and `rollback-planning`. Broaden trigger description. If merged word count exceeds ~3,000, apply progressive disclosure (split to `references/patterns.md` + `references/review-checklist.md`). The split may happen within commit 6 or as a follow-on commit — Logan's preference is that commit count not be artificially pinned at 15; if a clean separation makes review easier, do the conditional split as commit 6a (or shift the numbers). |
...
15 commits as the baseline sequence, each independently revertable. The exact count may grow by 1-2 if conditional splits (e.g., the consolidation overflow per §6.4) end up as separate commits — Logan's preference is that commits not be jammed together for the sake of a fixed count.
